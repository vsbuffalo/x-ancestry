...
\begin{equation}
  ...
  P_{HC}(R=r | \fsxa, k) = \sum_{s=r_\text{min}}^{k-1} \frac{ {s + 1 \choose k - s - 1 } { r - s \choose k + s - r } }{(\mathcal{F}_{k+1})^2}
\end{equation}
...
\begin{align}
  ...
  P(N=n | k)  =  \sum_{r=r_\text{HC min}}^{2k-1} P_{HC}(R=r | k) \left[  P(N = n | \fsxa, R=r) P(\fsxa | R=r) \; + \right. \nonumber \\
...
\begin{equation}
  ...
  \sum_{s=r_\text{min}}^{k-1}  {s + 1 \choose k - s + 1 } { r - s \choose k + s - r }
\end{equation}
%
...
% This probability is valid for $r_\text{FC min} \le r \le 2k-2$ and
...
\begin{equation}
  ...
  P(N = n | R=r) = \sum_{r=r_\text{FC min}}^{2k-2} \sum_{z=0}^{n} P(N = z | \fsxa) P(N = n - z | \msxa) P_{FC}(R=r)
\end{equation}
%
where $r_\text{FC min} = 2\lfloor (k-2)/2\rfloor + 2$, and 
%
\begin{align}
...
\begin{figure}[!ht]
  \centering
  \includegraphics[width=0.5\textwidth]{images/prob-xanc-n0.eps}
...
\begin{figure}[!ht]
  \centering
  \includegraphics[width=0.5\textwidth]{images/total-var-dist}
...
\begin{figure}[!ht]
  \centering
  \includegraphics[width=0.5\textwidth]{images/ped-collapse}
